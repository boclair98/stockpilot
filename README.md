...
| 실시간 학습 시세 | 학습센터도 공용 bootstrap 스냅샷을 사용하고 5초마다 백그라운드 갱신 |
| KOSPI 벤치마크 성장판 | 5·20거래일 시장 수익률, 상승·박스·하락·변동성 국면과 내 모의 포트폴리오의 상대성과를 익명 비교 |
...
| 다중 replica 안전성 | KIS 수집기·목표가 알림·보호주문 매처에 Redis lease를 적용하고 외부 호출 중 DB 잠금을 최소화 |
...
배포 전 백업·migration·Redis·장애 대응 기준은 [운영 런북](docs/OPERATIONS_RUNBOOK.md)에서 확인할 수 있습니다.
...
| 시장 | KOSPI 시장 나침반 | 5·20거래일 수익률, 일 변동성, 상승장·박스권·하락장·변동성 확대 국면 확인 |
...
| 거래 | 모의투자 운영센터 | 거래 가정·시장 세션·수수료·시세 지연 정책과 계좌 명세를 한 화면에서 확인하고 CSV로 저장 |
...
| 성장 | KOSPI 상대성과 | 로그인 후 최근 모의투자 수익률과 KOSPI를 같은 기간으로 비교하고 시장 대비 초과·미달 수익률 확인 |
...
### 02-1. KOSPI 시장 나침반과 벤치마크

KOSPI를 단순한 그래프가 아니라 모의투자의 기준선으로 사용합니다.

| 지표 | 의미 |
|---|---|
| 5거래일·20거래일 수익률 | 최근 시장 방향을 짧은 흐름과 한 달에 가까운 흐름으로 분리 |
| 시장 국면 | 수익률과 일별 변동성을 조합해 상승장·박스권·하락장·변동성 확대로 분류 |
| 내 모의투자 | 로그인한 사용자에게만 같은 기간의 포트폴리오 수익률을 계산 |
| 상대성과 | `내 수익률 - KOSPI 수익률`로 시장보다 앞섰는지·뒤처졌는지 표시 |

KOSPI와 개인 수익률 모두 종목명·수량·주문내역을 공개하지 않습니다. 데이터가 충분하지 않은 첫날에는
`기록을 모으는 중` 상태를 보여 주며, KIS의 마지막 정상 지수 스냅샷을 5분간 공유해 새로고침 폭주를 막습니다.
...
- 시장가 체결의 기준가 대비 비용과 슬리피지
- 대기 중인 지정가·조건 주문 취소
- 보유 종목별 익절가·손절가·보호 수량 등록 및 해제

보호주문은 서버가 약 5초마다 KIS 현재가를 확인해 익절 또는 손절 중 먼저 충족된 조건 하나만 실행합니다. 보호 수량은 일반 매도의 가능 수량에서 예약되어 중복 매도를 막습니다. 외부 증권사로 주문을 전송하지 않는 StockPilot 내부 가상주문입니다.
...
| 보호주문이 설정된 수량을 다시 매도 | 보호 수량까지 예약 수량에 포함해 초과·중복 매도 차단 |
...
주문창 아래의 `모의투자 운영센터`를 열면 사용자가 어떤 가정 위에서 거래하고 있는지와 현재 계좌 상태를 함께 확인할 수 있습니다.

- 시장가·지정가·손절·조건부 지정가의 체결 조건을 한눈에 확인
- 모의 수수료, 국내 매도 비용, 호가 스프레드·슬리피지 적용 기준 표시
- KRX·NXT·미국장 세션과 오래된 시세에서 조건부 주문을 보류하는 정책 표시
- 원화·달러 현금, 평가액, 보유 종목 수, 체결·대기·거절 주문 요약
- 실현손익·미실현손익·누적 비용과 서버 위험한도 확인
- 로그인 사용자는 보유 종목별 명세서를 CSV로 내려받아 학습 기록으로 보관

운영센터는 `GET /api/trading/rules`와 `GET /api/trading/statement`를 사용합니다. 공개 사용자는 초기 가상자금과 규칙만 보고, 개인 명세는 Google 로그인 이후에만 반환됩니다.
...
| 보호 범위 | 전체 보유 평가액 중 익절·손절 보호가 적용된 비율 | 보호 범위가 낮으면 위험관리 계획 등록 제안 |
...
| KOSPI 벤치마크·국면 | StockPilot 결정론적 계산 + KIS KOSPI 캐시 | 1분 화면 갱신·기준선 5분 | 데이터 부족 시 `WAITING`, 이전 정상 지수 유지 |
...
| `GET` | `/api/trading/rules` | 모의 체결·수수료·세션·시세 지연 규칙 | 없음 |
| `GET` | `/api/trading/statement` | 개인별 현금·평가액·보유·주문·손익·리스크 제한 명세 | 선택 |
...
| `GET` | `/api/growth/benchmark` | KOSPI 국면·5/20거래일 수익률·개인 상대성과(보유내역 비공개) | 선택 |
