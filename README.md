...
앱인토스 직접 진입 경로는 `/market`(종목 찾기), `/portfolio`(내 투자·복기), `/league`(수익률 리그)이며, 미니앱 주요 기능 등록은 콘솔 심사 단계에서 검증합니다. 실제 증권 주문·투자자문·수익 보장 기능은 없으며 모든 주문은 StockPilot 가상원장에서만 처리됩니다. 주문 전 비용과 주문 후 현금은 현재 기준가를 사용한 **추정치**로 실제 체결 수수료·슬리피지는 서버에서 최종 검증합니다. 콘솔 테스트 빌드와 실제 토스 앱 심사·출시는 별개 단계입니다.
...
- 앱인토스 상태: 전용 AIT 번들·익명 세션·모바일 핵심 흐름 구현 및 콘솔 테스트 빌드 등록, 실제 기기 재검증·검토 승인·공개 출시 전
