...
앱인토스는 별도 비공개 저장소에서 개발하며 이 저장소의 웹 화면 코드는 변경하지 않습니다. 공용 시세 API는 수집 작업이 일시 중단되어도 마지막 대표 종목 가격을 유지하고, 캐시가 없거나 오래되면 단일 잠금으로 KIS 종목 조회를 다시 시작합니다. 국내 종목은 KIS가 제공한 거래량과 시가총액(억원)을, 미국 종목은 제공되는 거래량만 표시합니다. 데이터가 없는 항목은 추정값으로 채우지 않습니다. 장외·주말에는 마지막 조회 가격이므로 실시간 체결가로 해석하면 안 됩니다. 리그 API는 시세 API와 별도로 동작하며, 앱에서도 독립적으로 표시됩니다.
...
| 시장 탐색 | KRX·NXT·미국 대표 종목, 선택한 시장에 맞는 상승 종목, 한국어 종목명·종목코드·티커 검색과 통신 오류 시 재시도 |
| 빠른 재방문 | 서버에 저장되는 관심종목(최대 20개)과 이 기기에만 저장되는 최근 본 종목(최대 8개)을 홈·시장 화면에 표시 |
| 시세 확인 | 주문창에 종목별 시세 기준 시각, KRX·NXT·미국의 정규 거래시간대 안내, 시세 새로고침 제공. 휴장일 판단과 실제 개장 상태로 오인하지 않도록 안내 |
| 가상주문 | 1·5·10주 빠른 선택, 주문 가능 금액, 예상 수수료·국내 매도세·주문 후 현금을 별도 확인 화면에 표시. 실제 슬리피지·체결가는 서버에서 결정 |
...
| 매매 기록·복기 | 최근 가상주문 종목에 대해 매매 이유·기대 기간·확신 정도를 비공개로 저장하고, 이후 결과와 차이를 기록. 기존 성장 허브 API를 재사용 |
...
앱인토스 직접 진입 경로는 `/market`(종목 찾기), `/portfolio`(내 투자·복기), `/league`(수익률 리그)입니다. 세 경로는 앱인토스 콘솔에 주요 기능으로 등록했으며 현재 검토 중입니다. 실제 증권 주문·투자자문·수익 보장 기능은 없으며 모든 주문은 StockPilot 가상원장에서만 처리됩니다. 주문 전 비용과 주문 후 현금은 현재 기준가를 사용한 **추정치**로 실제 체결 수수료·슬리피지는 서버에서 최종 검증합니다. 콘솔 테스트 빌드와 실제 토스 앱 심사·출시는 별개 단계입니다.
