...
| 가상주문 | 가격과 수량을 분리해 입력 중 시세 갱신 영향을 차단하고, 1·5·10주 빠른 선택·주문 가능 금액·가상 체결·실거래 없음·수수료 반영을 주문 전 표시 |
| 주문 보호 | 주문 가능 금액 초과 매수와 미보유·보유 수량 초과 매도를 제출 전에 차단하고, 주문 접수 성공과 이후 잔액 갱신 오류를 구분해 안내 |
| 주문 후 이동 | 주문 결과에서 `내 투자 보기`를 눌러 보유 자산과 주문 내역으로 바로 이동 |
...
앱인토스 딥링크 후보는 `intoss://stockpilot-kr/`, `/market`, `/portfolio`, `/league`입니다. 실제 증권 주문·투자자문·수익 보장 기능은 없으며 모든 주문은 StockPilot 가상원장에서만 처리됩니다. 주문 전 잔액 비교는 예상 주문금액 기준의 편의 기능이며 실제 체결 수수료·슬리피지는 서버에서 최종 검증합니다. 콘솔 테스트 빌드와 실제 토스 앱 심사·출시는 별개 단계입니다.
