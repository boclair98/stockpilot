...
앱인토스는 별도 비공개 저장소에서 개발하며 이 저장소의 웹 화면은 변경하지 않습니다. 신규 `/api/toss-game`은 한국·미국 대표 종목 20개의 **자체 생성 가상 가격**, 별도 원화·달러 가상계좌, 주문, 수익률 리그를 제공합니다. 30초 가격은 서버의 결정적 함수로 모든 사용자에게 동일하고, KIS 시세·실제 KOSPI·웹 원장을 읽지 않습니다. 아직 앱인토스 공개판에 반영된 상태는 아니며 테스트·심사를 거쳐야 합니다. 웹 서비스의 KIS 시세 사용 권한은 별도 확인이 필요합니다.
...
| 거래 | 거래 전 4단계 안전 확인 | 가상거래 여부, 시세 최신성·주문 방식, 수량·예상금액, 매도 가능 수량을 확인해야 주문을 확정 |
...
| 자산 | 리밸런싱 코치 | 안정·균형·성장형 목표 현금비중을 선택하고 종목별 현재/목표 비중과 가상 조정 수량을 계산 |
...
| 습관 | 5분 투자 루틴 | 홈에서 시장 확인·종목 선택·가상주문 진행도를 보여주고, 마지막에는 오늘의 차트 챌린지·투자일지 복기로 연결 |
...
기존 웹 서비스는 그대로 유지하고, 앱인토스 출시 소스는 비공개 `boclair98/stockpilot-apps-in-toss` 저장소에서 별도로 관리합니다. 이 저장소의 `apps-in-toss/`는 이전 코드 기록이며 새 앱인토스 출시 소스가 아닙니다. 신규 앱은 토스의 작은 화면·뒤로가기·안전영역에 맞춰 홈, 종목 검색, 가상주문, 포트폴리오, 수익률 리그를 구성했습니다.

| 항목 | 구현 내용 |
|---|---|
| 사용자 식별 | Apps in Toss의 익명 식별값을 서버가 단방향 UUID로 변환하고 30일 Bearer 세션 발급 |
| 첫 사용 흐름 | 홈에서 `종목 찾기 → 가상 주문 → 수익률 확인` 3단계 루틴과 약 3분 소요 시간을 안내 |
| 시장 탐색 | 한국·미국 대표 종목 20개의 이름·코드 검색, 직전 30초 대비 가상 상승 순위. 전체 상장 종목이나 실제 거래량 순위가 아님 |
| 빠른 재방문 | 이 기기에만 저장되는 관심종목·최근 본 종목을 홈·시장 화면에 표시 |
| 가상 가격 | 실제 거래소와 무관한 30초 단위 가상 가격·가상시장 지수·11/33분 게임 차트. 실제 시세·KOSPI·거래량·시총은 제공하지 않음 |
| 가상주문 | 1·5·10주 빠른 선택, 주문 가능 금액, 예상 게임 수수료·매도세·주문 후 현금을 별도 확인 화면에 표시. 서버 체결 시점의 가상 가격 적용 |
| 주문 보호 | 주문 가능 금액 초과 매수와 미보유·보유 수량 초과 매도를 제출 전에 차단하고, 주문 접수 성공과 이후 잔액 갱신 오류를 구분해 안내 |
| 주문 후 이동 | 주문 결과에서 `내 투자 보기` 또는 `이유 기록`을 눌러 자산 확인·매매 복기로 바로 이동 |
| 매매 기록·복기 | 최근 가상주문 종목에 대해 매매 이유·기대 기간·확신 정도를 이 기기에만 저장하고, 이후 결과와 차이를 기록 |
| 전략 미션 | 첫 주문·두 종목 분산·국내/미국 보유·이유 기록·리그 참여 5개 미션의 실제 진행 상태 표시 |
| 자산·리그 | 시장 수·수익 종목 수까지 요약하고, 종목은 숨긴 채 닉네임·순위·수익률만 공개 |
| 네이티브 경험 | safe-area, 토스 내비게이션 바, 햅틱, 화면 분석 이벤트를 지원하되 브리지 실패 시 핵심 기능은 유지 |
| 모바일 UI | 360px·390px 화면, 44px 이상 터치 영역, 하단 탭, 바텀시트, 스켈레톤·빈 상태·오류 상태 대응 |
| 개인정보 | Google 로그인 없이 익명 키만 사용하며, 앱 안에서 데이터 삭제 가능 |
| 광고 | 앱인토스 홈의 배너 광고 1개. 주문·확인 화면에는 광고를 배치하지 않음 |

앱인토스 직접 진입 경로는 `/market`(종목 찾기), `/portfolio`(내 투자·복기), `/league`(수익률 리그)입니다. 실제 증권 주문·투자자문·수익 보장 기능은 없습니다. 신규 게임 계좌는 기존 웹·이전 앱 버전의 원장과 분리되고 과거 데이터는 자동 이전되지 않습니다. 콘솔 테스트 빌드와 실제 토스 앱 심사·출시는 별개 단계입니다.
...
주문 버튼을 누르면 바로 체결하지 않고 확인 시트를 한 번 더 보여줍니다. 사용자는 다음 네 항목을 직접 확인해야 하며, 매수처럼 보유 수량 검증이 필요 없는 경우에는 해당 항목을 자동 통과시킵니다.

1. **가상거래 확인:** 실제 증권계좌로 주문이 전송되지 않는다는 점을 확인합니다.
2. **시세·주문 방식 확인:** 마지막 갱신 시각과 시장가·지정가·조건부 주문 방식을 확인합니다.
3. **수량·금액 확인:** 수량, 기준 가격, 예상 주문금액을 다시 확인합니다.
4. **매도 가능 수량 확인:** 보유 수량, 대기 매도주문과 보호주문을 제외한 실제 매도 가능 수량을 확인합니다.

이 확인 절차는 모바일에서도 한 손으로 조작할 수 있는 체크 카드로 제공되며, 네 항목을 모두 선택하기 전에는 확정 버튼이 비활성화됩니다. 이를 통해 실시간 가격이 움직이는 상황에서의 오입력과 모의투자·실거래 오인을 함께 줄입니다.
...
TOSS_ALLOWED_ORIGINS=https://stockpilot-kr.apps.tossmini.com,https://stockpilot-kr.private-apps.tossmini.com,https://stockpilot-kr.web.tossmini.com,https://stockpilot-kr.private-web.tossmini.com
DART_API_KEY=...
...
- 앱인토스 상태: `20260924-10` 테스트 빌드 등록과 실기기 확인 완료, 앱 버전·주요 기능 검토 요청 제출(검토 중). 승인과 공개 출시는 별개이며 아직 완료되지 않음
